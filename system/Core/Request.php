...
class Request
{
    ...
    public function __construct($get, $post, $cookies, $files, $environment)
    {
        ...
        $path_segs = trim($this->server('PATH_INFO'),'/');
        
        if(! empty($path_segs)) {
            $path_segs = explode("/", $path_segs);
            
            if(isset($path_segs[0]) && empty($path_segs[0])) {
                $path_segs = array_splice($path_segs, 1);
            }
            
            $num_path_segs = sizeof($path_segs);
            if( $num_path_segs > 0 && empty($path_segs[$num_path_segs-1])) {
                $path_segs = array_splice($path_segs, 0, $num_path_segs-2);
            }
            
            if(! empty($path_segs)) {
                foreach ($path_segs as $index => $value) {
                    $this->path_segments[$index+1] = $value;
                }
            }
        }
    ...
    public function path_segment($num)
    {
        if(isset($this->path_segments[$num])) {
            return $this->path_segments[$num];
        }
